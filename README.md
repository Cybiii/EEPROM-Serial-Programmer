...
- **Read EEPROM contents**: Dump EEPROM data to serial monitor in hex format
- **Write EEPROM data**: Program custom data patterns to EEPROM
- **Erase EEPROM**: Fill entire EEPROM with 0xFF (erase state)
- **Serial output**: View EEPROM contents in organized hex dump format
...
### Arduino to Shift Register (74HC595)
...
- Chip Enable (CE) should be tied to ground
- VCC and GND connected to power rails
...
3. **View programmed data** - The sketch will automatically program the sample data and display it
4. **Customize data** - Modify the `customData[]` array to program your own data
...
- **Data format**: 8-bit parallel data interface
- **Serial speed**: 57600 baud for reliable communication

## Troubleshooting

- **No output**: Check serial monitor baud rate (57600)
- **Wrong data**: Verify wiring connections, especially data lines
- **Write failures**: Ensure proper power supply and write timing
- **Address issues**: Check shift register connections and power
